...
import { GameState, GameSettings, Direction, Point, INITIAL_SNAKE_LENGTH, SCORE_PER_APPLE } from '../types';
...
export function useGameLogic(settings: GameSettings) {
  ...
  const moveSnake = useCallback(() => {
    ...
    setGameState(prevState => {
      const newHead = getNextHead(prevState.snake[0], prevState.direction, settings.gridSize);
      ...
      if (appleEaten !== -1) {
        ...
        newApples[appleEaten] = generateApple(settings.gridSize, newSnake, prevState.apples);
...
function initializeGame(settings: GameSettings): GameState {
  ...
  const initialApples = Array.from(
    { length: settings.appleCount },
...
function generateApple(gridSize: number, snake: Point[], existingApples: Point[]): Point {
  let apple: Point;
  do {
    apple = {
    ...
    existingApples.some(existing => existing.x === apple.x && existing.y === apple.y)
  );
  return apple;
}

function getNextHead(head: Point, direction: Direction, gridSize: number): Point {
  const movements = {
    UP: { x: 0, y: -1 },
    DOWN: { x: 0, y: 1 },
    LEFT: { x: -1, y: 0 },
  ...
  return {
    x: (head.x + movement.x + gridSize) % gridSize,
    y: (head.y + movement.y + gridSize) % gridSize
  };
}