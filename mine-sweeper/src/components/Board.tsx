...
export const Board: React.FC<BoardProps> = ({
  ...
  const chordReveal = (x: number, y: number) => {
    ...
    const flaggedCount = neighbors.reduce((count, { x: nx, y: ny }) => 
      count + (board[ny][nx].isFlagged ? 1 : 0), 0
    );
  ...
  const toggleFlag = (x: number, y: number) => {
    ...
    const newBoard = [...board];
    newBoard[y][x] = { ...newBoard[y][x], isFlagged: !newBoard[y][x].isFlagged };
    setBoard(newBoard);
  };
